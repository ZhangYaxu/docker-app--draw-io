...
@pytest.mark.parametrize('name', [
    '/bash-commands/docker-compose--container--kill.sh',
...
def test_that_required_files_are_existing(host, name):
    test_dir = os.environ['TESTS_DIRECTORY'] + '/test--docker-compose--container--kill'  # noqa: #501
...
def test_that_required_docker_container_is_existing(host):
    c = host.docker.get_containers(name='server--draw-io', status='exited')
